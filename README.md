...
Frontend (React + Vite): A modern UI for file uploads and live data tracking.
Backend (FastAPI): High-performance API that handles file parsing and task delegation.
Task Queue (Celery + Redis): Manages asynchronous jobs so the UI never freezes during API calls.
Database (PostgreSQL/SQLite): Stores enrichment batches and company results persistently.
🛠️ Tech Stack
